...
def generate_groupchat_id(uid1: str) -> str:
    ...
    salt = token_hex(32)
    hash = sha256(token_hex(8).encode()).hexdigest()

    return f'{GC_IDENTIFIER}{base_rid}/{uid1}+{salt}${hash}'
...
def generate_private_rid(uid1: str) -> str:
    ...
    salt = token_hex(64)
    hash = sha256(token_hex(16).encode()).hexdigest()

    return f'{PV_IDENTIFIER}{uid1}/{base_rid}/{salt}/{hash}'
