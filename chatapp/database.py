...
def init():
    ...
    users = database.create_table("users", [
        ...
        text('bio').default(''),
        text("password").allow_null(),  # Assuming you'd have a hashed password field
        integer('accept_tos').default(1)
    ])

    database.create_table("rooms", [
        text("room_name"),
        text("room_id").primary(),
        real("created_at"),
    ])

    database.create_table("user_room_relationship", [
        text("user_id").foreign("users/user_id"),
        text("room_id").foreign("rooms/room_id"),
        text('user_perm').default(None)
    ])

    m = database.create_table("messages", [
        text("content"),
        text("user_id").foreign("users/user_id"),
        text("room_id").foreign("rooms/room_id"),
